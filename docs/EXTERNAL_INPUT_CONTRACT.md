...
VIPLab publishes two deliberately separate image planes. The browser and
Taskplanner observer use the 5 Hz pass-through preview plane:

```text
/preview/cam_1/color/image_raw/compressed  sensor_msgs/msg/CompressedImage
/preview/cam_2/color/image_raw/compressed  sensor_msgs/msg/CompressedImage
/preview/cam_3/color/image_raw/compressed  sensor_msgs/msg/CompressedImage
/preview/cam_4/color/image_raw/compressed  sensor_msgs/msg/CompressedImage
/preview/flir/color/image_raw/compressed   sensor_msgs/msg/CompressedImage
```

The dedicated perception computer at `192.168.1.7` is the only LAN consumer
that needs the full-rate CAM3/CAM4 RGB-D acquisition plane. CAM1, CAM2, and
FLIR remain RGB-only. CAM3 and CAM4 additionally publish native depth,
CameraInfo, and the retained factory depth-to-color extrinsics:
...
The perception computer renders one shared two-camera Debug raster and one
small status document:
