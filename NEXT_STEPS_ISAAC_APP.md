...
export FASTRTPS_DEFAULT_PROFILES_FILE=/home/rlaalswo/ros_projects/taskplanner_ws/config/fastdds_udp.xml
...
```bash
cd /home/rlaalswo/ros_projects/taskplanner_ws
unset DOCKER_HOST
...
ENABLE_SYNTHETIC_SCENE_CAMERA=false \
VLM_MODE=real \
VLM_PUBLISH_PERIOD_SEC=5.0 \
VLM_IMAGE_STALE_SEC=3.0 \
SURGEON_ACTOR_MODE=none \
docker compose up -d --force-recreate taskplanner-runtime
```

Use a slower VLM period for local Qwen tests. A 1 second period can keep the GPU
near full utilization because each VLM request may take several seconds.
