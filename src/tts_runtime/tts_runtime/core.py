...
# TTS-only scheduling classes.  These are intentionally local to the playback
# owner so the public ROS contracts and robot-control admission are unchanged.
TTS_PRIORITY_AUTONOMOUS = 10
TTS_PRIORITY_NORMAL = 50
TTS_PRIORITY_SURGEON = 100
# Lifecycle announcements are generated by the manager-owned lifecycle
# helper.  Keeping this identity in the existing reply id lets the dispatcher
# apply a finish barrier without adding a second request/store schema.
LIFECYCLE_REPLY_ID_PREFIX = "lifecycle-announcement:"
# Execution marks only endpoint-accepted, run-scoped completion cleanup with
# this durable identity.  It may continue through the finish barrier without
# reopening ordinary tool or conversational speech.
COMPLETION_CLEANUP_REPLY_ID_PREFIX = "completion-cleanup-announcement:"
...
class ReplyRequest:
    ...
    # Higher values are spoken first once accepted by the durable dispatcher.
    # Keep this last/defaulted for compatibility with existing local callers.
...
def is_lifecycle_reply_id(reply_id: object) -> bool:
    """Return whether a durable reply is one of the lifecycle cues."""

    return str(reply_id or "").strip().startswith(LIFECYCLE_REPLY_ID_PREFIX)


def is_finishing_safe_reply_id(reply_id: object) -> bool:
    """Return whether one durable reply may survive the finish barrier."""

    normalized = str(reply_id or "").strip()
    return normalized.startswith(
        (LIFECYCLE_REPLY_ID_PREFIX, COMPLETION_CLEANUP_REPLY_ID_PREFIX)
    )
...
class DeterministicWavCache:
    ...
    def is_valid(self, cache_key: str) -> bool:
        """Return whether one already-materialized cache entry is playable."""

        path = self.path_for(cache_key)
        if not path.is_file():
            return False
        try:
            return wav_duration(path) > 0.0
...
class PlaybackStore:
    ...
    def __init__(self, path: Path | str) -> None:
        ...
        with self._lock:
            ...
            if "priority" not in columns:
                self._connection.execute(
                    "ALTER TABLE playback_jobs ADD COLUMN priority INTEGER NOT NULL DEFAULT 50"
                )
            ...
            # Keep the established store version for compatibility with the
            # existing local ledger; the additive priority column is migrated
            # by name above and needs no destructive version bump.
            if user_version < 2:
    ...
    def insert(
        ...
        if isinstance(request.priority, bool):
            ...
            try:
                ...
                self._connection.execute(
                    ...
                    INSERT INTO playback_jobs(
                        ...
                        priority, terminal, success, error_code, message, sequence,
    ...
    def prune_terminal_history(
        ...
        *,
        ...
        """Bound only terminal ledger history without touching live work.

        ``reply_id`` idempotency is meaningful for the active gateway/run
        scope.  Retaining the newest terminal rows keeps short restart/replay
        protection while an age-and-count bound prevents a long-running TTS
        owner from turning its durable ledger into an unbounded text archive.
        Queued, waiting, and playing jobs are intentionally never candidates.
        """
        ...
        if not math.isfinite(now):
            raise ValueError("now_unix_sec must be finite")
        ...
        with self._lock:
            ...
            try:
                rows = self._connection.execute(
                    """
                    SELECT reply_id, updated_unix_sec
                      FROM playback_jobs
                     WHERE terminal = 1
                ...
                retired = [
                    ...
                    if index >= keep_recent or float(row["updated_unix_sec"]) <= cutoff
                ]
                if retired:
                    self._connection.executemany(
                        "DELETE FROM playback_jobs WHERE reply_id = ? AND terminal = 1",
                        ((reply_id,) for reply_id in retired),
                    )
                self._connection.execute("COMMIT")
            except BaseException:
                self._connection.execute("ROLLBACK")
    ...
    def fail_pending_for_procedure_finishing(
        self,
        *,
        gateway_instance_id: str,
        procedure_run_id: str,
    ) -> list[PlaybackEvent]:
        """Fail pending audio that is not part of the admitted finish flow.

        Lifecycle cues and execution-owned completion cleanup remain eligible.
        All other queued or admission-gated work is stale once the manager has
        entered its finishing state.
        """

        gateway = str(gateway_instance_id or "").strip()
        run_id = str(procedure_run_id or "").strip()
        if not gateway or not run_id:
            raise ValueError("an exact gateway/procedure scope is required")
        with self._lock:
            rows = self._connection.execute(
                """
                SELECT reply_id, state
                  FROM playback_jobs
                 WHERE gateway_instance_id = ?
                   AND procedure_run_id = ?
                   AND state IN ('queued', 'waiting')
                   AND terminal = 0
                   AND reply_id NOT LIKE ?
                   AND reply_id NOT LIKE ?
                 ORDER BY sequence ASC
                """,
                (
                    gateway,
                    run_id,
                    f"{LIFECYCLE_REPLY_ID_PREFIX}%",
                    f"{COMPLETION_CLEANUP_REPLY_ID_PREFIX}%",
                ),
            ).fetchall()
        ...
        for row in rows:
            event = self.transition(
                ...
                state="failed",
                error_code="procedure_finishing_barrier",
                message="non_lifecycle_tts_suppressed_after_procedure_finishing",
            )
...
class PlaybackDispatcher:
    ...
    def __init__(
        ...
        *,
        ...
        self._queue: PriorityQueue[tuple[int, int, str | object]] = PriorityQueue()
        self._lifecycle_lock = threading.Lock()
        self._scope_lock = threading.RLock()
        self._active_gateway_instance = ""
        self._active_procedure_run = ""
        self._playing_reply_id = ""
        self._playing_priority = TTS_PRIORITY_NORMAL
        self._playing_gateway_instance = ""
        self._playing_procedure_run = ""
        self._started = False
        self._stopping = threading.Event()
        self._worker: threading.Thread | None = None
        self._preempted_reply_ids: set[str] = set()
        self._finishing_scope: tuple[str, str] | None = None
        self._precomputed_cache_config: RuntimeConfig | None = None
        self._precomputed_cache_texts: frozenset[str] = frozenset()

    @staticmethod
    def _queue_item(event: PlaybackEvent) -> tuple[int, int, str]:
        """Order higher-priority speech first, FIFO within one priority."""

        return (-int(event.priority), int(event.sequence), event.reply_id)

    def configure_precomputed_cache(
        self,
        *,
        config: RuntimeConfig | None,
        texts: list[str] | tuple[str, ...],
    ) -> None:
        """Select optional high-quality cache entries for exact text matches."""

        with self._scope_lock:
            self._precomputed_cache_config = config
            self._precomputed_cache_texts = frozenset(
        ...
        with self._scope_lock:
            ...
            with self._lifecycle_lock:
                ...
                for reply_id in queued_ids:
                    event = self.store.get(reply_id)
                    if event is not None:
                        self._queue.put(self._queue_item(event))
    ...
    def is_procedure_finishing(
        ...
        """Return whether one exact run has crossed the finish barrier."""
        ...
        with self._scope_lock:
            return self._finishing_scope == scope

    def begin_procedure_finishing(
        self,
        gateway_instance_id: str,
        procedure_run_id: str,
    ) -> list[PlaybackEvent]:
        """Fence stale speech when the manager enters procedure finishing.

        The barrier is run-scoped and idempotent.  It removes pending
        non-lifecycle jobs and interrupts a currently playing non-lifecycle
        job, while preserving lifecycle cues for the finish/stop/record
        sequence.
        """
        ...
        with self._scope_lock:
            if self.active_scope != scope:
                return emitted
            self._finishing_scope = scope
            emitted.extend(
                self.store.fail_pending_for_procedure_finishing(
                    gateway_instance_id=scope[0],
                    procedure_run_id=scope[1],
                )
            )
            if self._playing_reply_id and not is_finishing_safe_reply_id(
                self._playing_reply_id
            ):
                interrupted = self.store.transition(
                    ...
                    state="failed",
                    error_code="procedure_finishing_barrier",
                    message="non_lifecycle_tts_interrupted_after_procedure_finishing",
                )
    ...
    def set_active_procedure_scope(
        ...
        with self._scope_lock:
            ...
            if previous_scope != (active_gateway, active_run):
                self._finishing_scope = None
    ...
    def prewarm(
        ...
        *,
        ...
        """Best-effort materialize fixed WAVs without delaying worker start.

        TTS starts prewarming in a background thread.  Holding the lifecycle
        lock across model inference used to make the first GatewayInfo (and
        therefore surgery start) wait for every cache phrase.  Check the
        lifecycle briefly between phrases instead: a worker start simply ends
        any remaining warmup and immediately owns normal playback.
        """

        with self._lifecycle_lock:
            if self._started:
                return []
        warm_config = self.config if config is None else config
        warm_synthesizer = self.synthesizer if synthesizer is None else synthesizer
        ...
        for raw_text in texts:
            ...
            if not text or text in seen:
                continue
            seen.add(text)
            key = cache_key_for(text, warm_config)
            wav_path, synthesis, cache_hit = self.cache.materialize(
                cache_key=key,
                text=text,
                synthesizer=warm_synthesizer,
            )
            results.append(
                PrewarmResult(
                    text=text,
    ...
    def submit(self, request: ReplyRequest) -> EnqueueResult:
        with self._scope_lock:
            ...
            if (
                self._finishing_scope
                == (self._active_gateway_instance, self._active_procedure_run)
                and not is_finishing_safe_reply_id(request.reply_id)
            ):
                raise ValueError(
                    "procedure finishing barrier suppresses non-lifecycle TTS"
                )
            key = cache_key_for(request.text, self.config)
            with self._scope_lock:
                warm_config = self._precomputed_cache_config
                warm_texts = self._precomputed_cache_texts
            if warm_config is not None and request.text.strip() in warm_texts:
                warm_key = cache_key_for(request.text, warm_config)
                if self.cache.is_valid(warm_key):
                    key = warm_key
            ...
            if result.accepted:
                ...
                if result.event.state == "queued":
                    self._queue.put(self._queue_item(result.event))
                    if (
                        self._playing_reply_id
                        and result.event.priority > self._playing_priority
                    ):
                        interrupt = getattr(self.player, "interrupt", None)
                        if callable(interrupt) and interrupt():
                            self._preempted_reply_ids.add(self._playing_reply_id)
            return result
    ...
    def release_waiting(self, reply_id: str) -> PlaybackEvent | None:
        ...
        with self._scope_lock:
            ...
            if (
                self._finishing_scope
                == (self._active_gateway_instance, self._active_procedure_run)
                and not is_finishing_safe_reply_id(current.reply_id)
            ):
    ...
    def _process(self, reply_id: str) -> None:
        ...
        with self._scope_lock:
            ...
            if (
                self._finishing_scope
                == (self._active_gateway_instance, self._active_procedure_run)
                and not is_finishing_safe_reply_id(latest.reply_id)
            ):
                scope_failure = self.store.transition(
                    reply_id,
                    expected_state="queued",
            ...
            if (
                ...
                if playing is not None:
                    ...
                    self._playing_priority = latest.priority
        ...
        try:
            ...
            with self._scope_lock:
                preempted = reply_id in self._preempted_reply_ids
                self._preempted_reply_ids.discard(reply_id)
                self._fail(
                    reply_id,
                    expected_state="playing",
                    error_code=(
        ...
        with self._scope_lock:
            ...
            if preempted:
                terminal = self.store.transition(
                    ...
                    expected_state="playing",
                    state="failed",
                    error_code="preempted_by_higher_priority",
                    message="playback_preempted_by_higher_priority_request",
                )
            elif (
                self._active_gateway_instance
    ...
    def stop(self, timeout_sec: float = 10.0) -> None:
        ...
        with self._scope_lock:
            with self._lifecycle_lock:
                ...
                if started:
                    self._stopping.set()
                    self._queue.put((1_000_000, 1_000_000, self._STOP))
