#!/usr/bin/env python3
"""Smoke-test real-VLM connectivity and ontology scope at every runtime phase.

This is not a phase-accuracy benchmark. The selected runtime phase is present in
the VLM context, so top-phase agreement is diagnostic only.
"""
...
class RuntimeSweep(Node):
    def __init__(self) -> None:
        super().__init__("real_vlm_runtime_sweep")
    ...
    def run_phase(
        ...
        return {
            ...
            "checks": checks,
            "diagnostic_vlm_top_phase_matches": top_phase == phase,
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--report-path", default="reports/real_vlm_runtime_sweep_latest.json"
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = {
        "mode": "real_vlm_runtime_context_smoke",
        "purpose": "connectivity, health, schema, and procedure-scope validation",
        "phase_accuracy_claim_valid": False,
        "phase_accuracy_limitation": (
            "The selected runtime phase is included in VLM context; top-phase "
            "agreement is diagnostic and must not be reported as recognition accuracy."
        ),
        "model_ids": sorted(
            {
                str(row.get("model_id", ""))
                for row in rows
                if str(row.get("model_id", ""))
            }
        ),
